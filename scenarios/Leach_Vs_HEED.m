...
SX = W/2; SY = L/2;  % Position of sink
...
for r=1:num_rounds % iterating on each round
    ...
    %calcul delay
    delay_leach = calculerDelayLeach(N, p, r);
...
for r=1:num_rounds % iterating on each round
    ...
    %calcul delay
    delay = calculerDelayLeach(N, p, r);
...
plot(T,EHdis,'r-h')
plot(T,EHdis,'r-h',T_L,EHdis_L,'b-o');
xlabel('Temps (s)'); ylabel('Énergie (j)')
...
% to save figure
print(gcf,'./Énergietotaledissipée.png','-dpng')  
% to save figure almost exactly what it looks on your screen, use: 
figure_size = get(gcf,'position');
set(gcf,'PaperPosition',figure_size/100); 
print(gcf,'-dpng','-r100', ['./Énergietotaledissipée.png']);


figure('Position',[298 66 792 613]);
plot(T,AliveH,'-x')
plot(T,AliveH,'-x',T_L,AliveH_L,'-x'); 
xlabel('Temps (s)'); ylabel('No of live Sensors nodes')
title('Life time of sensor nodes')
legend('HEED','LEACH')

% to save figure
print(gcf,'./Lifetimeofsensornodes.png','-dpng')  
% to save figure almost exactly what it looks on your screen, use: 
figure_size = get(gcf,'position');
set(gcf,'PaperPosition',figure_size/100); 
print(gcf,'-dpng','-r100', ['./Lifetimeofsensornodes.png']);

figure('Position',[511 61 792 613]);
plot(T,BitsH,'-x')
plot(T,BitsH,'-x',T_L,BitsH_L,'-x'); 
xlabel('Temps (s)'); ylabel('Débit (nombre de paquets)')
title(['Débit (' num2str(N) ' Nœuds de capteurs)'])
legend('HEED','LEACH')
% to save figure
print(gcf,'./Débit.png','-dpng')  
% to save figure almost exactly what it looks on your screen, use: 
figure_size = get(gcf,'position');
set(gcf,'PaperPosition',figure_size/100); 
print(gcf,'-dpng','-r100', ['./Débit.png']);
